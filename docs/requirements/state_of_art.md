# Regent System. Аналіз предметної області
...
## Підходи та способи вирішення завдання
  ...
  * #### Розпізнавання іменованої сутності
    Не менш популярна задача NLP - розпізнавання іменованих сутностей (Named-entity recognition, NER) із великого об'єму текстової інформації. Тобто задача NEP - отримання суті тексту (ключових слів), за допомогою якої можна отримати потрібну інформацію в базі данних та передати її користувачу. Постановка завдання NER дуже гнучка. Можна виділяти будь-які потрібні безперервні фрагменти тексту, які чимось відрізняються від решти. В результаті можна підібрати свій набір сутностей для конкретної практичної задачі, обробити тексти цим набором і навчити модель. Такий сценарій зустрічається повсюдно, і це робить NER однією з найбільш часто виконуваних завдань NLP в індустрії.
     Як обробляється природна мова?
Деякі завдання NLP для природної мови, на відміну від обробки зображень, до недавніх пір вирішувалися за допомогою класичних алгоритмів машинного навчання. Для вирішення більшості завдань потрібний ретельний вибір архітектури, а також ручне збирання та обробка ознак. Однак останнім часом нейронні мережі почали давати більш точні результати в порівнянні з класичними моделями і сформували загальний підхід для вирішення завдань NLP.
  * #### Конвеєр NLP
    Реалізація будь-якої складної задачі зазвичай означає побудову пайплайна (конвеєра). Суть цього підходу в тому, щоб розбити задачу на ряд послідовних подзадач і вирішувати кожну з них окремо. У побудові пайплайна можна умовно виділити дві частини: обробка вхідних даних (зазвичай займає найбільше часу) і побудова моделі. Основних етапів - сім.
1. Перші два кроки пайплайна, які виконуються для вирішення практично будь-яких завдань NLP, - це сегментація (розподіл тексту на пропозиції) і токенізація (розподіл пропозицій на токени, тобто окремі слова).
2. Обчислення ознак кожного токена. Обчислюються контекстно-незалежні ознаки токена. Це набір ознак, що не залежать від сусідніх з токеном слів.
3. Визначення значущості і фільтрація стоп-слів. У російській, українській та англійській мовах дуже багато допоміжних слів, наприклад «and», «the», «a». При статистичному аналізі тексту ці маркери створюють багато шуму, так як з'являються частіше, ніж інші. Тому їх відзначають як стоп-слова і відсівають.
4. Дозвіл кореференціі. У російській, українській та англійській мовах дуже багато займенників типу he, she, it або ти, я, він і т. д. Це скорочення, якими ми замінюємо на листі справжні імена і назви. Людина може простежити взаємозв'язок цих слів від пропозиції до пропозиції, грунтуючись на контексті. Але NLP-модель не знає, що означають займенники, адже вона розглядає лише одне речення за раз.
5. Парсінг залежностей. Кінцева мета цього кроку - побудова дерева, в якому кожен токен має єдиного батька. Коренем може бути головне дієслово. Також потрібно встановити тип зв'язку між двома словами.
6. Переклад обробленого тексту в векторну форму. Даний крок дозволяє сформувати векторні уявлення слів. Таким чином, у слів, які використовуються в одному і тому ж контексті, схожі вектори.
7. Побудова моделі в залежності від поставленої мети. Наприклад, модель для класифікації або генерації нових текстів.
 ### __Word2Vec__
Word2vec - загальна назва для сукупності моделей на основі штучних нейронних мереж, призначених для отримання векторних уявлень слів на природній мові. 
У сфері медіа аналізу даний класифікатор оцінює тональність (визначення полярності емоційних оцінок в досліджуваному тексті) як повідомлення в цілому, так і в ставленні до конкретного об'єкта (компанії / бренду / персони), про що йдеться в повідомленні. На вхід мережі подається текст, перетворений в матрицю, рядки якої відповідають ознакам, витягнутим з тексту, а стовпці - компонентам «семантичних» векторів. В якості ознак використовуються різні фрагменти тексту: слова, поєднання слів, окремі послідовності символів. В ході експериментів була обрана модель нейромережі, яка використовує шари довгої короткостроковій пам'яті (LSTM), яка показала найкращі результати. Ключовим фактором для підвищення якості прогнозів стала комбінація різних моделей векторних уявлень тих самих ознак тексту (embeddings), побудованих за допомогою Word2Vec. Такі «знання» дозволяють враховувати особливості тональної оцінки для сутностей з відмінними семантичними властивостями, в тому числі коректно обробляти випадки, коли один і той же контекст несе протилежну за тональністю оцінку.
